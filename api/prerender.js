...
const BOT_AGENTS = /googlebot|bingbot|slurp|duckduckbot|baiduspider|yandexbot|sogou|facebot|ia_archiver|twitterbot|linkedinbot|whatsapp|telegrambot|applebot/i;
...
export default async function handler(request) {
    ...
    // Extrai slug — suporta tanto /blog/:slug quanto ?slug=
    const pathParts = url.pathname.split('/').filter(Boolean);
    const slug = pathParts[1] || url.searchParams.get('slug');

    if (!isBot || !slug) {
        // Para usuários normais retorna 200 com HTML mínimo que carrega o SPA
        return new Response(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Natuclinic</title></head><body><div id="root"></div><script type="module" src="/src/main.jsx"></script></body></html>`, {
